    ...
    for (let element of form.children) {
      if (element.tagName.toLowerCase() == "input") {
        if (element.value == null || element.value == "") {
          element.setCustomValidity("This field shouldn'nt be empty!");
        } else {
...
function validate(form) {
  ...
  for (let element of form.children) {
    if (element.tagName.toLowerCase() === "input" && element.type !== "radio") {
      if (!element.value) {
        element.setCustomValidity("This field shouldn’t be empty!");
        isValid = false;
      } else {
        element.setCustomValidity("");
      }
  ...
  const readValue = document.querySelector('input[name="read"]:checked');
  if (!readValue) {
    document.querySelector("label[for='true']").textContent =
      "Please select a read status!";
    isValid = false;
  }
